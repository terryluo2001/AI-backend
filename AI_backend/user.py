...
def register(request):
    ...
    if request.method == 'POST':
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO user (username, email, hashed_password, topic_weights) VALUES (%s, %s, %s, %s)",
                (username, email, encrypted_password, topic_weights)
            )  
            conn.commit()
...
def onboard(request):
    ...
    if request.method == 'PATCH':
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE user SET topic_weights = %s WHERE username = %s",
            (topic_weights, username)
        )  
